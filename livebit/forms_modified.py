...
from livebit.models import User, Photo, PhotoLikes, Followers, PhotoTag
...
class AjaxLogin(Ajax):
	def validate(self):
		...
		if not User.objects.filter(email=self.email).exists() or not User.objects.filter(username=self.email).exists():
			return None, self.error("Email or password is incorrect.")

		if not check_password(self.password, User.objects.filter(email=self.email)[0].password) or not check_password(self.password, User.objects.filter(username=self.email)[0].password):
			return None, self.error("Email or password is incorrect.")

		if '@' in self.email:
			u = User.objects.filter(email=self.email)[0]
		else:
			u = User.objects.filter(username=self.email)[0]
